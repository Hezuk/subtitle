...
- **작업 취소·재시도** — 진행 중 언제든 취소, 번역 실패 시 한국어 자막 유지 후 재시도, 인코딩 실패 시 자막 수정 후 재시도
...
`hezuk.com`, `www.hezuk.com` 같은 실제 도메인을 연결하려면 nginx를 FastAPI 앞단에 두는 방식이 가장 단순합니다.

- nginx 설정 예시: [`deploy/nginx/hezuk.com.conf`](/Users/sunghoonkim/claude/subtitle/deploy/nginx/hezuk.com.conf)
- systemd 서비스 예시: [`deploy/systemd/subtitle.service`](/Users/sunghoonkim/claude/subtitle/deploy/systemd/subtitle.service)
- 배포 가이드: [`docs/nginx-deployment.md`](/Users/sunghoonkim/claude/subtitle/docs/nginx-deployment.md)
